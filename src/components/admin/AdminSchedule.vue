<template>
  <div class="space-y-3">
    <div class="flex items-center justify-between">
      <p class="text-xs text-slate-500">Интервал слотов: 10 минут</p>
        ...
        <div class="flex items-center gap-3">
          ...
          <label class="relative flex-shrink-0 cursor-pointer">
            ...
            <div class="w-10 h-6 rounded-full transition-colors duration-200"
                 :class="day.is_open ? 'bg-brand-500' : 'bg-surface-muted border border-surface-border'">
          ...
          <!-- Day name -->
          <span class="w-24 text-sm font-semibold flex-shrink-0"
    ...
    <!-- Preview -->
    <div v-if="!loading && todaySchedule" class="card p-3">
      <p class="text-xs text-slate-500 mb-2">Слоты на сегодня ({{ dayNames[todaySchedule.day] }})</p>
      <div v-if="todaySchedule.is_open" class="flex flex-wrap gap-1.5">
        <span v-for="slot in todaySlots" :key="slot"
              class="text-xs px-2 py-1 bg-brand-500/10 text-brand-400 rounded-lg font-mono">
          {{ slot }}
        </span>
      </div>
...
const todaySlots = computed(() => {
  ...
  return generateSlots(day.open_time, day.close_time)
})
...
function slotCount(day: ScheduleDay): number {
  ...
  return generateSlots(day.open_time, day.close_time).length
}
...
async function save() {
  ...
  haptic('medium')
  try {
    await scheduleApi.update(schedule.value)
    notify('success')
  } catch {
    notify('error')
  } finally {
...
onMounted(async () => {
  try {
    ...
    // Гарантируем правильный порядок дней
    schedule.value = dayOrder.map(d =>
        data.find(x => x.day === d) ?? { day: d, open_time: '10:00', close_time: '18:00', is_open: false }
    )
  } finally {
    loading.value = false
  }
...
.time-input {
  ...
  text-sm text-slate-200 outline-none focus:border-brand-500 transition-colors
